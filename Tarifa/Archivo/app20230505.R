  ...
  dashboardBody(
    ...
    fluidPage(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("Selecionar cobertura", uiOutput("selesht")),
                  tabPanel("Selecionar CSVs", uiOutput("selefile"))
    )
  )
)
)
...
# the main function to create the SASRT CSVs
  ...
  *",names(tabla_fac_list)[k],";
  ...
  output$downloadTARIFA <- downloadHandler(
    ...
    content = function(file) {
      file.copy(file.path(ruta_inicial,'www','Tarifa.xlsx'), file,overwrite = TRUE)
    }
  )
