...
output$selevocafic <- renderUI({
  ...
  observeEvent(input$Ejecutarvoca,{
    ...
    VOCA_DATA<-transmute(VOCAB_SHEET(),
                         ...
                         VOCAB_TERM_INPUT_EXCLUDE_FLG = ifelse(VOCAB_SHEET()[[4]]=="N","Y","N"),
                         VOCAB_TERM_OUTPUT_EXCLUDE_FLG = ifelse(VOCAB_SHEET()[[5]]=="N","Y","N"),
...
# the main function to create the SASRT CSVs
  ...
  *",names(tabla_fac_list)[k],";
  ...
  observeEvent(input$Ejecutarformula,{
    ...
    posibleLU<-function(palabra, mont=3000){
      perm<-c()
      for(k in 1:mont){
        perm<-c(perm,
                paste(
                  ifelse(runif(nchar(strsplit(palabra,"")[[1]]))>0.5,
                         tolower(strsplit(palabra,"")[[1]]), 
                         toupper(strsplit(palabra,"")[[1]])), 
                  collapse = ""))
      }
      unique(perm)
    }
    ...
    posi_then<-paste(paste0("\\s+",posibleLU("then"),"\\s+"),collapse = "|")
    posi_and<-paste(paste0("\\s+",posibleLU("and"),"\\s+"),collapse = "|")
    posi_if<-paste(paste0("\\s*",posibleLU("if"),"\\s+"),collapse = "|")
    posi_elseif<-paste(paste0("\\s+",posibleLU("elseif"),"\\s+"),collapse = "|")
    posi_else<-paste(paste0("\\s+",posibleLU("else"),"\\s+"),collapse = "|")
    ...
    for(k in 1:length(FORMULAS)){
      ...
      cont_sent<-formu  %>%  str_split(paste(posi_then, posi_and, posi_if, posi_elseif, posi_else, sep = "|")) %>%
        unlist()%>% str_trim() %>% str_subset(".") %>% str_remove_all(";")
      cont_logi<-formu %>%  str_extract_all(paste(posi_then, posi_and, posi_if, posi_elseif, posi_else, sep = "|")) %>% 
        unlist()%>% str_trim()%>% str_to_upper()
      ...
      #EXPRESSION ORDER
      exp_order<-c()
      for(i in 1:length(cont_logi)){
        if(cont_logi[i] %in% c("IF", "ELSEIF", "ACT", "THEN","ELSE")){
          exp_order<-c(exp_order,1)
        }
        else if(cont_logi[i] %in% c("AND")){
          exp_order<-c(exp_order, exp_order[i-1]+1)
        }
      }
      ...
      for(i in 1:length(cha)){
        if(toupper(cha[i])=="IF"){nn<-nn+1; rule_seq<-c(rule_seq, nn)}
        else if(toupper(cha[i])=="ACT"){nn<-nn+1; rule_seq<-c(rule_seq, nn)}
        else if(toupper(cha[i])=="ELSEIF"){nn<-nn+1; rule_seq<-c(rule_seq,nn)}
        else if(toupper(cha[i])=="AND"){rule_seq<-c(rule_seq,nn)}
        else if(toupper(cha[i])=="THEN"){rule_seq<-c(rule_seq,nn)}
        else if(toupper(cha[i])=="ELSE"){nn<-nn+1; rule_seq<-c(rule_seq,nn)}
      }
      
      chaRT<-c()
      for(i in 1:length(cha)){
        if(toupper(cha[i])=="IF"){chaRT<-c(chaRT, "if")}
        else if(toupper(cha[i])=="ACT"){chaRT<-c(chaRT, "if")}
        else if(toupper(cha[i])=="AND"){chaRT<-c(chaRT, chaRT[i-1])}
        else if(toupper(cha[i])=="ELSEIF"){chaRT<-c(chaRT, "elseif")}
        else if(toupper(cha[i])=="THEN"){chaRT<-c(chaRT, chaRT[i-1])}
        else if(toupper(cha[i])=="ELSE"){chaRT<-c(chaRT, "elseif")}
      }
      
  
      
      #hay que corregir sent2aux symbol para que sea compatible formulas de primas y formulas de multifactor
      # condition_sentence<-function(logi, sentence){
      #   
      #   if(logi %in% c("IF","ELSEIF","AND")){
      #     asent1<-sentence %>% str_extract("(\\s*[\\w|[:punct:]]+\\s*)(?=[:symbol:])") %>% str_trim()
      #     asent2aux<-sentence %>% str_extract("(?<=[:symbol:])(\\s*\\w+[\\w|[:graph:]\\s]*\\s*)") %>% str_trim()
      ...
      #     
      #     asent1<-sentence %>% str_extract("(\\s*[\\w|[:punct:]]+\\s*)(?=[:symbol:])") %>% str_trim()
      #     asent2<-sentence %>% str_extract("(?<=[:symbol:])(\\s*[\\w|[:graph:]\\s]*\\s*)") %>% str_trim()
      #   }
      #   
      #   c(asent1, asent2)  
      # }
      condition_sentence<-function(logi, sentence){
        ...
        sentence<-sentence %>% str_remove_all("\\s+")
        if(logi %in% c("IF","ELSEIF","AND")){
          
          asent1<-sentence %>% str_extract("(\\s*[\\w|[:punct:]]+\\s*)(?=[:symbol:])") %>% str_trim()
          #derecha de simbolo < > = tiene que empezar con un w+ o '
          asent2aux<-sentence %>% str_extract("(?<=(\\>|\\<|\\=))(\\s*[\\'\\w+][\\w|[:graph:]\\s]*\\s*)") %>% str_trim()
          asymbol<-sentence %>% str_extract("[:symbol:]+") %>% str_trim()
          asent2<-paste0(asymbol,asent2aux)
        }
        else if(logi %in% c("ACT","THEN", "ELSE")){
      ...
      formula_base_list[[k]]<-data.frame(
        ...
        EXPRESSION = ifelse(str_detect(condtion_sentence_comb[,2],"\\w+\\(.+\\)"), paste0('"',condtion_sentence_comb[,2],'"'),condtion_sentence_comb[,2]),
        EXPRESSION_ORDER = exp_order,
    ...
    formula_base_list[[1]]$RULE_SET_PATH[1] %>% str_split("/") %>% .[[1]] %>% .[length(.)]
  ...
  %BRM_IMPORT_RULESET (
  CSV   =
    %STR(//CONTROL_TECNICO/", paste(VERSION_FOLDER,.$RULE_SET_PATH[1] %>% str_split("/") %>% .[[1]] %>% .[length(.)], .$RULE_SET_NM[1],"RULESET_FORMULA.CSV", sep="_"),"),
  REJECT=
    %STR(//CONTROL_TECNICO/", "REJ_",paste(VERSION_FOLDER,.$RULE_SET_PATH[1] %>% str_split("/") %>% .[[1]] %>% .[length(.)], .$RULE_SET_NM[1],"RULESET_FORMULA.CSV", sep="_"),")
);"))


    SASCODES<-paste(SASCODE %>% unlist(), collapse = " \n ")

#nombre hay que modificar para identificar mas 
    ...
    walk(formula_base_list, ~write.csv(.,paste(VERSION_FOLDER,.$RULE_SET_PATH[1] %>% str_split("/") %>% .[[1]] %>% .[length(.)],.$RULE_SET_NM[1],"RULESET_FORMULA.CSV", sep="_"), quote = F, row.names = F))
    #====================================================================================
...
############################_RULE_FLOW_#########################################################################################################
  ...
  observeEvent(input$Ejecutarflow,{
    ...
    conten <- flowsheet[5:nrow(flowsheet),2:ncol(flowsheet)]
    col_odd <- seq_len(ncol(conten)) %% 2   
    conten[,col_odd==0]
    
    RULE_FLOW_NAME <- as.character(conten[,col_odd==0][1,]) 
    RULE_FLOW_FOLDER <- as.character(conten[,col_odd==0][2,]) 
    ...
    RULE_SET_FOLDER <- conten[,col_odd==1] %>% .[3:nrow(.),] %>% as.matrix() %>% as.character() %>% .[!is.na(.)]
    RULE_SET_PATH <- paste(MAIN_FOLDER,VERSION_FOLDER, RULE_SET_FOLDER, sep = "/")
    
    PATH_NM<-conten[,col_odd==0] %>% .[3:nrow(.),]
    names(PATH_NM)<-RULE_FLOW_PATH
    
    FLOW_NM<-conten[,col_odd==0] %>% .[3:nrow(.),]
    ...
    RULE_FLOW_FILE<-cbind(FNAME_SNAME,FPATH_SNAME, data.frame(SPATH=RULE_SET_PATH)) %>%
      mutate(RULE_FLOW_SK = ".",
             RULE_FLOW_NM = FNAME,
      ...
      select(-FNAME,-SNAME,-FPATH,-SPATH) %>% 
